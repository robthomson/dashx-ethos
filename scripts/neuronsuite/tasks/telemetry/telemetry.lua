...
local simSensors = neuronsuite.utils.simSensors
local round = neuronsuite.utils.round
...
local sensorTable = {
    ...
    armflags = {
        ...
        onchange = function(value)
            neuronsuite.session.isArmed = (value == 1 or value == 3)
        end,
    },
    ...
    temp_esc = {
        ...
        localizations = function(value, paramMin, paramMax, paramThresholds)
            ...
            local prefs = neuronsuite.preferences.localizations
...
local function checkCondition(sensorEntry)
    local sess = neuronsuite.session
...
function telemetry.getSensorSource(name)
    ...
    elseif neuronsuite.session.telemetryType == "crsf" then
    ...
    elseif neuronsuite.session.telemetryType == "sport" then
        protocol = "sport"
...
function telemetry.validateSensors(returnValid)
    ...
    if not neuronsuite.session.telemetryState then
        if not memo_listSensors then build_memo_lists() end
...
function telemetry.active()
    return neuronsuite.session.telemetryState or false
end
