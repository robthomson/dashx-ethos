...
function adjfunc.wakeup()
  local now = os.clock()
  local events = neuronsuite.preferences and neuronsuite.preferences.events
  if not (events and (events.adj_f or events.adj_v)) then return end
  if (now - initTime) < 5 then return end

  adjfuncAdjValue = neuronsuite.tasks.telemetry.getSensor("adj_v")
  adjfuncAdjFunction = neuronsuite.tasks.telemetry.getSensor("adj_f")
...
if adjfuncPendingFuncAnnounce and not firstRun and events.adj_f then
          ...
          if wavs then
            local playFile = neuronsuite.utils and neuronsuite.utils.playFile
            if playFile then
              for i = 1, #wavs do
