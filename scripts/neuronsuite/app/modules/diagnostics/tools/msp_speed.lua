...
local function getMSP()
    ...
    local avgQueryTime = neuronsuite.utils.round(mspQueryTimeCount / mspSpeedTestStats['total'], 2) .. "s"
...
local function startTest(duration)
    ...
    testLoader = form.openProgressDialog({
        ...
        wakeup = function()
            ...
            if neuronsuite.session.telemetryState == false and startTest == true and system:getVersion().simulation ~= true then
                if testLoader then
                    testLoader:close()
                    testLoader = nil
                end
            ...
            -- do msp query
            if neuronsuite.tasks.msp.mspQueue:isProcessed() and ((now - RateLimit) >= Rate) then
                RateLimit = now
...
local function openPage(pidx, title, script)
    ...
    neuronsuite.app.formNavigationFields['menu'] = form.addButton(line, {x = x - 5 - buttonW - buttonWs, y = neuronsuite.app.radio.linePaddingTop, w = buttonW, h = neuronsuite.app.radio.navbuttonHeight}, {
    ...
    -- ACTION BUTTON
    neuronsuite.app.formNavigationFields['tool'] = form.addButton(line, {x = x - buttonWs, y = neuronsuite.app.radio.linePaddingTop, w = buttonWs, h = neuronsuite.app.radio.navbuttonHeight}, {
        text = "*",
        icon = nil,
        options = FONT_S,
        press = function()
    ...
    local posText = {x = x - 5 - buttonW - buttonWs - 5 - buttonWs, y = neuronsuite.app.radio.linePaddingTop, w = 200, h = neuronsuite.app.radio.navbuttonHeight}

    line['rf'] = form.addLine(i18n("app.modules.msp_speed.rf_protocol"))
    fields['rf'] = form.addStaticText(line['rf'], posText, string.upper(neuronsuite.tasks.msp.protocol.mspProtocol))
