...
function elrs.setFblSensors(list)
  ...
  -- We need sid to map neuronsuite id -> sidElrs (load, then free)
  local sidList = getSidList()
  if sidList then
...
elrs.setFblSensors(neuronsuite.session.telemetryConfig)
...
function elrs.crossfirePop()
  ...
  if (CRSF_PAUSE_TELEMETRY == true or neuronsuite.app.triggers.mspBusy == true or not telemetryActive()) then
...
function elrs.wakeup()
  ...
  if fp ~= _lastSlotsFp then
    _lastSlotsFp = fp
    elrs.setFblSensors(neuronsuite.session.telemetryConfig or {})
    resetSensors() -- clears local caches so new ones get created on demand
  end

  if telemetryActive() and neuronsuite.session.telemetrySensor then
    local n = 0
    while elrs.crossfirePop() do
      n = n + 1
      if n >= 50 then break end
      if CRSF_PAUSE_TELEMETRY == true or neuronsuite.app.triggers.mspBusy == true then break end
    end
    refreshStaleSensors()
  else
    resetSensors()
