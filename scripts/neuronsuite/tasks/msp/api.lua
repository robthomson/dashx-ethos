...
local apidir = "SCRIPTS:/".. neuronsuite.config.baseDir  .. "/tasks/msp/api/"
...
local function loadAPI(apiName)
    ...
    if firstLoadAPI then
        mspHelper = neuronsuite.tasks.msp.mspHelper
        utils = neuronsuite.utils
        callback = neuronsuite.tasks.callback
        firstLoadAPI = false
    end    
...
local function parseMSPChunk(buf, structure, state)
    ...
    while state.index <= #structure and processedFields < 5 do  -- Process 5 fields per wakeup
        ...
        if field.apiVersion and neuronsuite.utils.apiVersionCompare("<", field.apiVersion) then
            goto continue
        end
...
function apiLoader.calculateMinBytes(structure)
    ...
    for _, param in ipairs(structure) do
        ...
        if not param.apiVersion or neuronsuite.utils.apiVersionCompare(">=", param.apiVersion) then
            insert_param = true
        end
...
function apiLoader.buildWritePayload(apiname, payload, api_structure, noDelta)
    if not neuronsuite.app.Page then
        utils.log("[buildWritePayload] No page context available", "info")
        return nil
    end

    local positionmap = neuronsuite.app.Page.apidata and neuronsuite.app.Page.apidata.positionmap[apiname]
    local receivedBytes = neuronsuite.app.Page.apidata and neuronsuite.app.Page.apidata.receivedBytes[apiname]
    local receivedBytesCount = neuronsuite.app.Page.apidata and neuronsuite.app.Page.apidata.receivedBytesCount[apiname]
...
function apiLoader.buildDeltaPayload(apiname, payload, api_structure, positionmap, receivedBytes, receivedBytesCount)
    ...
    for idx, formField in ipairs(neuronsuite.app.formFields) do
        local pageField = neuronsuite.app.Page.fields[idx]
    ...
    local actual_fields = {}
    if neuronsuite.app.Page and neuronsuite.app.Page.apidata then
        for _, field in ipairs(neuronsuite.app.Page.apidata.formdata.fields) do
...
function apiLoader.buildFullPayload(apiname, payload, api_structure)
    ...
    for _, field_def in ipairs(api_structure) do
        ...
        if not actual_field and field_def.decimals then
            scale = scale / neuronsuite.app.utils.decimalInc(field_def.decimals)
        end
        value = math.floor(value * scale + 0.5)
...
function apiLoader.prepareStructureData(structure)
    ...
    for _, param in ipairs(structure) do
        if param.apiVersion and neuronsuite.utils.apiVersionCompare("<", param.apiVersion) then
            goto continue
        end
