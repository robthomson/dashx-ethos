...
local smartfuel = assert(neuronsuite.compiler.loadfile("tasks/sensors/lib/smartfuel.lua"))()
local smartfuelvoltage = assert(neuronsuite.compiler.loadfile("tasks/sensors/lib/smartfuelvoltage.lua"))()
...
local function calculateFuel()
    ...
    if neuronsuite.session.modelPreferences and neuronsuite.session.modelPreferences.battery and neuronsuite.session.modelPreferences.battery.calc_local then
         if neuronsuite.session.modelPreferences.battery.calc_local == 1 then
...
local function calculateConsumption()
            ...
            if neuronsuite.session.modelPreferences and neuronsuite.session.modelPreferences.battery and neuronsuite.session.modelPreferences.battery.calc_local then
                if neuronsuite.session.modelPreferences.battery.calc_local == 1 then
                    local capacity = (neuronsuite.session.batteryConfig and neuronsuite.session.batteryConfig.batteryCapacity) or 1000 -- Default to 1000mAh if not set
                    local smartfuelPct = neuronsuite.tasks.telemetry.getSensor("smartfuel")
                    local warningPercentage = (neuronsuite.session.batteryConfig and neuronsuite.session.batteryConfig.consumptionWarningPercentage) or 30
                    ...
                    return neuronsuite.tasks.telemetry.getSensor("consumption") or 0
                end
            else
...
local function createOrUpdateSensor(appId, fieldMeta, value)
    ...
    local currentModule = neuronsuite.session.telemetrySensor and neuronsuite.session.telemetrySensor:module()
    ...
    if not sensorCache[appId] and not negativeCache[appId] then
        ...
        if existingSensor then
            ...
            sensor:appId(appId)
            sensor:physId(0)
            sensor:module(neuronsuite.session.telemetrySensor:module())
...
function smart.wakeup()
    ...
    if firstWakeup then
        log = neuronsuite.utils.log
        tasks = neuronsuite.tasks
        firstWakeup = false
    end

    -- If telemetry is inactive, clear caches & bail
    if not (neuronsuite.session.telemetryState and neuronsuite.session.telemetrySensor) then
        sensorCache = {}
        negativeCache = {}
        lastValue = {}
        return
