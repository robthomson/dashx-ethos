...
local baseDir = neuronsuite.config.baseDir
local preferences = neuronsuite.config.preferences
...
dashboard.flightmode = neuronsuite.flightmode.current or "preflight"
...
function dashboard.computeOverlayMessage()
    ...
    if neuronsuite.session.apiVersion and neuronsuite.session.rfVersion and not neuronsuite.session.isConnectedLow and state ~= "postflight" then
        if system.getVersion().simulation == true then
            return pad .. "SIM " .. neuronsuite.session.apiVersion .. pad
        else
            return pad .. "RF" .. neuronsuite.session.rfVersion .. pad
        end
    end

    -- 4) LAST: generic waiting message (don’t let it mask actionable errors)
    if not neuronsuite.session.isConnectedHigh and state ~= "postflight" then
...
function dashboard.renderLayout(widget, config)
    ...
    if layout.showgrid or neuronsuite.preferences.developer.overlaygrid then
    ...
    if layout.showstats or neuronsuite.preferences.developer.overlaystats then
        local headerOffset = (isFullScreen and headerLayout and headerLayout.height) or 0

        local cpuUsage = neuronsuite.session and neuronsuite.session.cpuload or 0
        local ramUsage = neuronsuite.session and neuronsuite.session.freeram or 0

        lcd.font(FONT_S)

        local cpuText = "CPU: " .. neuronsuite.utils.round(cpuUsage, 0) .. "%"
        local ramText = "RAM: " .. neuronsuite.utils.round(ramUsage, 0) .. "kB"
...
local function getThemeForState(state)
    local prefs = neuronsuite.session.modelPreferences and neuronsuite.session.modelPreferences.dashboard
    local fallback = neuronsuite.preferences.dashboard
...
local function getThemeForState(state)
    local modelPrefs = neuronsuite.session.modelPreferences and neuronsuite.session.modelPreferences.dashboard
    local userPrefs = neuronsuite.preferences.dashboard
    local val = nil
    if modelPrefs then
...
function dashboard.create()
    ...
    if not dashboard.loaders then
        dashboard.loaders = assert(compile("SCRIPTS:/" .. neuronsuite.config.baseDir .. "/widgets/dashboard/lib/loaders.lua"))()
    end

    -- 2) ensure user theme dir exists
    os.mkdir("SCRIPTS:/" .. neuronsuite.config.preferences .. "/dashboard/")
...
function dashboard.event(widget, category, value, x, y)
    ...
    if state == "postflight" and category == EVT_KEY and value == 131 then
        neuronsuite.widgets.dashboard.flightmode = "preflight"
...
function dashboard.wakeup(widget)
    ...
    if firstWakeupCustomTheme and
        neuronsuite.session.mcu_id and
        neuronsuite.session.modelPreferences and
        neuronsuite.session.modelPreferences.dashboard then

        local modelPrefs = neuronsuite.session.modelPreferences.dashboard
        local currentPrefs = neuronsuite.preferences.dashboard
    ...
    local admin = neuronsuite.app and neuronsuite.app.guiIsRunning 

    -- Throttle CPU usage based on connection and visibility
    if not neuronsuite.session.isConnected then
    ...
    local currentFlightMode = neuronsuite.flightmode.current or "preflight"
                ...
                if utils.dir_exists(basePath, folder) then
                    ...
                    if chunk then
                        ...
                        if ok and initTable and type(initTable.name) == "string" then
                            if not initTable.developer or neuronsuite.preferences.developer.devtools == true then
                                num = num + 1
                                themes[num] = {
                                    name = initTable.name,
...
function dashboard.getPreference(key)
    if not neuronsuite.session.modelPreferences or not dashboard.currentWidgetPath then return nil end

    if not neuronsuite.app.guiIsRunning then
        return neuronsuite.ini.getvalue(neuronsuite.session.modelPreferences, dashboard.currentWidgetPath, key)
    else
        return neuronsuite.ini.getvalue(neuronsuite.session.modelPreferences, neuronsuite.app.dashboardEditingTheme, key)
    end
end
...
function dashboard.savePreference(key, value)
    if not neuronsuite.session.modelPreferences or not neuronsuite.session.modelPreferencesFile or not dashboard.currentWidgetPath then
        return false
    end
    if not neuronsuite.app.guiIsRunning then
        neuronsuite.ini.setvalue(neuronsuite.session.modelPreferences, dashboard.currentWidgetPath, key, value)
        return neuronsuite.ini.save_ini_file(neuronsuite.session.modelPreferencesFile, neuronsuite.session.modelPreferences)
    else
        neuronsuite.ini.setvalue(neuronsuite.session.modelPreferences, neuronsuite.app.dashboardEditingTheme, key, value)
        return neuronsuite.ini.save_ini_file(neuronsuite.session.modelPreferencesFile, neuronsuite.session.modelPreferences)
    end
end
