...
local ch = neuronsuite.currentSbusServoIndex
local ch_str = i18n("app.modules.sbusout.ch_prefix") .. tostring(ch + 1)
local offset = 6 * ch -- 6 bytes per channel

local servoCount = neuronsuite.session.servoCount or 6
local motorCount = 1
if neuronsuite.session.tailMode == 0 then motorCount = 2 end
...
local function saveServoSettings(self)
    ...
    local mixIndex = neuronsuite.currentSbusServoIndex
    local mixType = math.floor(neuronsuite.app.Page.fields[1].value)
    local mixSource = math.floor(neuronsuite.app.Page.fields[2].value)
    local mixMin = math.floor(neuronsuite.app.Page.fields[3].value)
    local mixMax = math.floor(neuronsuite.app.Page.fields[4].value)
    ...
    neuronsuite.tasks.msp.mspHelper.writeU8(message.payload, mixIndex)
    neuronsuite.tasks.msp.mspHelper.writeU8(message.payload, mixType)
    neuronsuite.tasks.msp.mspHelper.writeU8(message.payload, mixSource)
    neuronsuite.tasks.msp.mspHelper.writeU16(message.payload, mixMin)
    neuronsuite.tasks.msp.mspHelper.writeU16(message.payload, mixMax)
    -- neuronsuite.tasks.msp.mspHelper.writeU8(message.payload, frameRate)
...
local function onSaveMenuProgress()
    neuronsuite.app.ui.progressDisplay(i18n("app.modules.sbusout.saving"), i18n("app.modules.sbusout.saving_data"))
    saveServoSettings()
...
local function setMinMaxIndex(self)
    minMaxIndex = math.floor(neuronsuite.app.Page.fields[1].value + 1) 
    ...
    if firstLoad == true then
        ...
        neuronsuite.app.Page.fields[2].value = 0
        neuronsuite.app.Page.fields[3].value = defaultMin
        neuronsuite.app.Page.fields[4].value = defaultMax
...
local function postLoad(self)
    ...
    -- sbus_out_frame_rate = neuronsuite.app.Page.values[#neuronsuite.app.Page.values]
...
local function onNavMenu(self)
    ...
    neuronsuite.app.ui.openPage(neuronsuite.app.lastIdx, neuronsuite.app.lastTitle, "sbusout/sbusout.lua")
...
local function wakeup()
    ...
    if enableWakeup == true then
        ...
        -- set min and max values
         if neuronsuite.app.Page.fields[2].value and neuronsuite.app.Page.fields[2].value >= currentSourceMax then neuronsuite.app.Page.fields[2].value = currentSourceMax end

        -- handle min value
         if neuronsuite.app.Page.fields[3].value and neuronsuite.app.Page.fields[3].value <= currentMin then neuronsuite.app.Page.fields[3].value = currentMin end
         if neuronsuite.app.Page.fields[3].value and neuronsuite.app.Page.fields[3].value >= currentMax then neuronsuite.app.Page.fields[3].value = currentMax end

        -- handle max value
         if neuronsuite.app.Page.fields[4].value and neuronsuite.app.Page.fields[4].value >= currentMax then neuronsuite.app.Page.fields[4].value = currentMax end
         if neuronsuite.app.Page.fields[4].value and neuronsuite.app.Page.fields[4].value <= currentMin then neuronsuite.app.Page.fields[4].value = currentMin end
