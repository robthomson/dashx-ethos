...
function ui.openPageDashboard(idx, title, script, source, folder)
    ...
    dashx.app.formNavigationFields['menu'] = form.addButton(line, {x = x, y = dashx.app.radio.linePaddingTop, w = buttonW, h = dashx.app.radio.navbuttonHeight}, {
        ...
        press = function()
            ...
            dashx.app.ui.openPage(pageIdx, "@i18n(app.modules.settings.dashboard)@", "settings/tools/dashboard_settings.lua")
        end
    })
...
function ui.navigationButtons(x, y, w, h)
    ...
    if navButtons.tool ~= nil and navButtons.tool == true then
        dashx.app.formNavigationFields['tool'] = form.addButton(line, {x = toolOffset, y = y, w = wS, h = h}, {text = "@i18n(app.navigation_tools)@", icon = nil, options = FONT_S, paint = function() end, press = function() dashx.app.Page.onToolMenu() end})
    end
    ...
    if navButtons.help ~= nil and navButtons.help == true then
        ...
        if help then
            ...
            dashx.app.formNavigationFields['help'] = form.addButton(line, {x = helpOffset, y = y, w = wS, h = h}, {text = "@i18n(app.navigation_help)@", icon = nil, options = FONT_S, paint = function() end, press = function() end})
...
function ui.openPageHelp(txtData, section)
    ...
    form.openDialog({width = dashx.session.lcdWidth, title = "Help - " .. dashx.app.lastTitle, message = message, buttons = {{label = "@i18n(app.btn_close)@", action = function() return true end}}, options = TEXT_LEFT})
end
