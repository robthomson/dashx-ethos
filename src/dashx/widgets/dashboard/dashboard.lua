...
local function paintObjects()
    local module = loadedStates[currentState]
    if not module then
        dashboard.utils.screenError("@i18n(widgets.dashboard.theme_load_error)@", true, 0.5)
        return
    end
...
function dashboard.paint()
    if unsupportedResolution then
        dashboard.utils.screenError("@i18n(widgets.dashboard.unsupported_resolution)@", true, 0.5)
        return
    end
...
function dashboard.wakeup(widget)
    ...
    local dirty = wakeObjects()
    if dirty or runtimeState.flightmode_changed or (now - lastInvalidateAt) >= invalidateInterval then
