...
-- Configuration: expects neuronsuite.config to be globally available
local logTimings = true
if neuronsuite and neuronsuite.config then
  if type(neuronsuite.preferences.developer.compilerTiming) == "boolean" then
    logTimings = neuronsuite.preferences.developer.compilerTiming or false
  end
end
...
function compile.wakeup()
  ...
  if #compile._queue > 0 then
    ...
    if neuronsuite and neuronsuite.utils and log then
      if ok then
        neuronsuite.utils.log("Deferred-compiled (throttled): " .. entry.script, "info")
      else
        neuronsuite.utils.log("Deferred-compile error: " .. tostring(err), "debug")
      end
    end
  end
end
...
function compile.loadfile(script, opts)
  ...
  if loader then
    ...
    if not neuronsuite.preferences.developer.compile then
...
function compile.clearAll()
  ...
  lru_cache.order  = {}
  lru_cache.pinned = {}
  if neuronsuite and neuronsuite.utils then
    neuronsuite.utils.log("Cleared all cached scripts (" .. total .. " evicted)", "info")
  end
end
