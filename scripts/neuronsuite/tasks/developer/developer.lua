...
function developer.wakeup()
    ...
    -- This is an example of how to use the api library to query the governor mode
        neuronsuite.utils.log("API Debug Task: GOVERNOR_CONFIG", "info")
    ...
    --[[
    neuronsuite.utils.log("API Debug Task: TELEMETRY_CONFIG", "info")
    ...
    if neuronsuite.session.mcu_id and neuronsuite.config.preferences then
        local iniName = "SCRIPTS:/" .. neuronsuite.config.preferences .. "/models/" .. neuronsuite.session.mcu_id ..".ini"
        local api = neuronsuite.tasks.ini.api.load("api_template")
