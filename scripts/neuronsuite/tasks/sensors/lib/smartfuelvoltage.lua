...
local telemetry
local currentMode = neuronsuite.flightmode.current or "preflight"
...
local function getStickLoadFactor()
    local rx = neuronsuite.session.rx.values
...
local function applySagCompensation(voltage)
    ...
    local multiplier = neuronsuite.session.modelPreferences and neuronsuite.session.modelPreferences.battery and neuronsuite.session.modelPreferences.battery.sag_multiplier or 0.7
...
local function fuelPercentageCalcByVoltage(voltage, cellCount)
    local bc = neuronsuite.session.batteryConfig
...
local function smartFuelCalc()
    if not telemetry then
        telemetry = neuronsuite.tasks.telemetry
    end

    if not neuronsuite.session.isConnected or not neuronsuite.session.batteryConfig then
        resetVoltageTracking()
        return nil
    end

    -- make sure we reset the method if the sensor mode changes
    if neuronsuite.session.modelPreferences and neuronsuite.session.modelPreferences.battery and neuronsuite.session.modelPreferences.battery.calc_local then
        if lastSensorMode ~= neuronsuite.session.modelPreferences.battery.calc_local then
            resetVoltageTracking()
            lastSensorMode = neuronsuite.session.modelPreferences.battery.calc_local
        end
    end
    ...
    if currentMode ~= lastMode then
        neuronsuite.utils.log("Flight mode changed – resetting voltage state", "info")
        resetVoltageTracking()
    ...
    if not voltageStabilised then
        if isVoltageStable() then
            neuronsuite.utils.log("Voltage stabilized at: " .. voltage, "info")
            voltageStabilised = true
        else
            neuronsuite.utils.log("Waiting for voltage to stabilize...", "info")
            return nil
        end
    end

    if #lastVoltages >= 1 and neuronsuite.flightmode.current == "preflight" then
        local prev = lastVoltages[#lastVoltages - 1]
        if voltage > prev + voltageThreshold then
            neuronsuite.utils.log("Voltage increased after stabilization – resetting...", "info")
            resetVoltageTracking()
            stabilizeNotBefore = os.clock() + preStabiliseDelay
            return nil
        end
    ...
    local now = os.clock()
    if (neuronsuite.flightmode.current == "inflight" or neuronsuite.flightmode.current == "postflight")
