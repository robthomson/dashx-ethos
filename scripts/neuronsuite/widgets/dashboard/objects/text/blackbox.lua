...
function render.dirty(box)
    if not neuronsuite.session.telemetryState then return false end
...
local function eraseBlackboxAsk()
    ...
    form.openDialog({
        title =  neuronsuite.i18n.get("widgets.bbl.erase_dataflash"),
        message = neuronsuite.i18n.get("widgets.bbl.erase_dataflash") .. "?",
        buttons = buttons,
        options = TEXT_LEFT
    })
end
...
local function eraseDataflash()
    ...
    progress = form.openProgressDialog(
        neuronsuite.i18n.get("app.msg_saving"),
        neuronsuite.i18n.get("app.msg_saving_to_fbl")
    )
    progress:value(0)
...
local function ensureCfg(box)
    local theme_version = (neuronsuite and neuronsuite.theme and neuronsuite.theme.version) or 0
...
function render.wakeup(box)
    ...
    -- Extract telemetry
    local totalSize = neuronsuite.session.bblSize
    local usedSize  = neuronsuite.session.bblUsed
    ...
    if totalSize and usedSize then
        ...
        displayValue = string.format("%." .. cfg.decimals .. "f/%." .. cfg.decimals .. "f %s",
            transformedUsed, transformedTotal, neuronsuite.i18n.get("app.modules.fblstatus.megabyte"))
    else
