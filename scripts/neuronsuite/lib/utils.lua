...
local compiler = neuronsuite.compiler 
...
function utils.session()
    ...
    --neuronsuite.utils.log("Session initialized", "info")
end


--- Checks if the RX map is ready by verifying the presence of required channel mappings.
-- The function returns true if the `neuronsuite.session.rx.map` table exists and at least one of the following fields is present:
...
function utils.rxmapReady()
    if neuronsuite.session.rx and neuronsuite.session.rx.map and (neuronsuite.session.rx.map.collective or neuronsuite.session.rx.map.elevator or neuronsuite.session.rx.map.throttle or neuronsuite.session.rx.map.rudder) then
        return true
    end
    return false
end
...
function utils.inFlight()
    if neuronsuite.flightmode.current == "inflight" then
        return true
    end
    return false
end
...
function utils.msp_version_array_to_indexed()
    local arr = {}
    local tbl = neuronsuite.config.supportedMspApiVersion or {"12.06", "12.07", "12.08"}
...
function utils.getGovernorState(value)
    ...
    if not neuronsuite.tasks.telemetry then
    ...
    if neuronsuite.session and neuronsuite.session.apiVersion and neuronsuite.utils.apiVersionCompare(">", "12.07") then
        local armflags = neuronsuite.tasks.telemetry.getSensor("armflags")
    ...
    local armdisableflags = neuronsuite.tasks.telemetry.getSensor("armdisableflags")
...
function utils.createCacheFile(tbl, path, options)
    ...
    if not f then
        neuronsuite.utils.log("Error creating cache file: " .. err, "info")
        return
    end
...
function utils.playFile(pkg, file)
    ...
    local wavUser    = "SCRIPTS:/neuronsuite.user/audio/user/" .. pkg .. "/" .. file
    local wavLocale  = "SCRIPTS:/neuronsuite.user/audio/" .. av .. "/" .. pkg .. "/" .. file
    local wavDefault = "SCRIPTS:/neuronsuite/audio/en/default/" .. pkg .. "/" .. file

    -- Determine which file to play: user → locale → default
    local path
    if neuronsuite.utils.file_exists(wavUser) then
        path = wavUser
    elseif neuronsuite.utils.file_exists(wavLocale) then
        path = wavLocale
    else
        path = wavDefault
    end
...
function utils.ethosVersionAtLeast(targetVersion)
    ...
    if targetVersion == nil then
        if neuronsuite and neuronsuite.config and neuronsuite.config.ethosVersion then
            targetVersion = neuronsuite.config.ethosVersion
        else
            -- Fail-safe: if no targetVersion is provided and config is missing
            return false
        end
    elseif type(targetVersion) == "number" then
        neuronsuite.utils.log("WARNING: utils.ethosVersionAtLeast() called with a number instead of a table (" .. targetVersion .. ")", 2)
        return false
    end
...
function utils.log(msg, level)
    if neuronsuite.tasks and neuronsuite.tasks.logger then
        neuronsuite.tasks.logger.add(msg, level or "debug")
    end
end
...
function utils.findModules()
    ...
    for _, v in pairs(system.listFiles(modules_path)) do
        if v ~= ".." and v ~= "." and not v:match("%.%a+$") then
            ...
            if not func then
                neuronsuite.utils.log("Failed to load module init " .. init_path .. ": " .. err, "info")
            else
                local ok, mconfig = pcall(func)
                if not ok then
                    neuronsuite.utils.log("Error executing " .. init_path .. ": " .. mconfig, "info")
                elseif type(mconfig) ~= "table" or not mconfig.script then
                    neuronsuite.utils.log("Invalid configuration in " .. init_path, "info")
                else
                    neuronsuite.utils.log("Loading module " .. v, "debug")
                    mconfig.folder = v
...
function utils.findWidgets()
    ...
    for _, v in pairs(system.listFiles(widgets_path)) do
        if v ~= ".." and v ~= "." and not v:match("%.%a+$") then
            ...
            local func, err = compiler.loadfile(init_path)
            if not func then
                neuronsuite.utils.log("Failed to load widget init " .. init_path .. ": " .. err, "debug")
            else
                local ok, wconfig = pcall(func)
                if not ok then
                    neuronsuite.utils.log("Error executing widget init " .. init_path .. ": " .. wconfig, "debug")
                elseif type(wconfig) ~= "table" or not wconfig.key then
                    neuronsuite.utils.log("Invalid configuration in " .. init_path, "debug")
                else
                    wconfig.folder = v
    ...
    local function getCachedBitmap(key, tryPaths)
        ...
        if not path then
            for _, p in ipairs(tryPaths) do
                if neuronsuite.utils.file_exists(p) then
                    path = p
                    break
                end
            end
...
function utils.simSensors(id)
    os.mkdir("LOGS:")
    os.mkdir("LOGS:/neuronsuite")
    os.mkdir("LOGS:/neuronsuite/sensors")
...
function utils.logMsp(cmd, rwState, buf, err)
    if neuronsuite.preferences.developer.logmsp then
        local payload = neuronsuite.utils.joinTableItems(buf, ", ")
        neuronsuite.utils.log(rwState .. " [" .. cmd .. "]{" .. payload .. "}", "info")
        if err then
            neuronsuite.utils.log("Error: " .. err, "info")
        end
    end
end

function utils.reportMemoryUsage(location)
    if not neuronsuite.preferences.developer.memstats then return end
    location = location or "Unknown"

    local cpuInfo = neuronsuite.session and neuronsuite.session.cpuload or 0
    ...
    neuronsuite.utils.log(string.format("[%s] CPU Load: %.2f ", location, neuronsuite.utils.round(cpuInfo,0)) .. "%", "info")
    neuronsuite.utils.log(string.format("[%s] Main stack available: %.2f KB", location, mainStackKB), "info")
    neuronsuite.utils.log(string.format("[%s] System RAM available: %.2f KB", location, ramKB), "info")
    neuronsuite.utils.log(string.format("[%s] Lua RAM available: %.2f KB", location, luaRamKB), "info")
    neuronsuite.utils.log(string.format("[%s] Lua Bitmap RAM available: %.2f KB", location, luaBitmapsRamKB), "info")
end
...
function utils.onReboot()
    neuronsuite.utils.log("utils.onReboot called", "info")
    neuronsuite.session.resetSensors    = true
    neuronsuite.session.resetTelemetry  = true
    neuronsuite.session.resetMSP        = true
...
function utils.apiVersionCompare(op, req)
    ...
    local a, b = parts(neuronsuite.session.apiVersion or 12.06), parts(req)
...
function utils.muteSensorLostWarnings()
    if neuronsuite.session.telemetryModule then
        local module = neuronsuite.session.telemetryModule
