...
local function createOrUpdateSensor(appId, fieldMeta, value)
    ...
    local currentModule = neuronsuite.session.telemetrySensor
                        and neuronsuite.session.telemetrySensor:module()
...
local function updateSessionField(meta, value)
    if not meta.sessionname or type(neuronsuite.session) ~= "table" then return end
    local t = neuronsuite.session
...
function msp.wakeup()
    ...
    if neuronsuite.session.resetMSPSensors then
        sensorCache   = {}
        negativeCache = {}
        lastValue     = {}
        lastPush      = {}
        neuronsuite.session.resetMSPSensors = false
    end
    ...
    local isAdmin = neuronsuite.app.guiIsRunning
