...
function render.dirty(box)
  ...
  -- threshold in degrees/pixels to avoid repaints on tiny deltas
  local eps = 0.2
  local ppd = (box._cfg and box._cfg.ppd) or 2.0
  local pitchPxDelta = math.abs(((box._lastPitch or d.pitch) - d.pitch) * ppd)
  local rollDelta    = math.abs((box._lastRoll  or d.roll)  - d.roll)
  local yawDelta     = math.abs((box._lastYaw   or d.yaw)   - d.yaw)

  box._lastPitch, box._lastRoll, box._lastYaw = d.pitch, d.roll, d.yaw
  return pitchPxDelta > 0.5 or rollDelta > eps or yawDelta > eps
end
