...
local function openPage(pidx, title, script)
    ...
    if neuronsuite.app.gfx_buttons["governor"] == nil then neuronsuite.app.gfx_buttons["governor"] = {} end
    if neuronsuite.preferences.menulastselected["governor"] == nil then neuronsuite.preferences.menulastselected["governor"] = 1 end
    ...
    for pidx, pvalue in ipairs(S_PAGES) do
        ...
        if neuronsuite.preferences.general.iconsize ~= 0 then
            if neuronsuite.app.gfx_buttons["governor"][pidx] == nil then neuronsuite.app.gfx_buttons["governor"][pidx] = lcd.loadMask("app/modules/governor/gfx/" .. pvalue.image) end
        else
            neuronsuite.app.gfx_buttons["governor"][pidx] = nil
        end

        neuronsuite.app.formFields[pidx] = form.addButton(line, {x = bx, y = y, w = buttonW, h = buttonH}, {
            text = pvalue.name,
            icon = neuronsuite.app.gfx_buttons["governor"][pidx],
            options = FONT_S,
            paint = function()
            end,
            press = function()
                neuronsuite.preferences.menulastselected["governor"] = pidx
                neuronsuite.app.ui.progressDisplay()
                local name = i18n("app.modules.governor.name") .. " / " .. pvalue.name
                neuronsuite.app.ui.openPage(pidx, name, "governor/tools/" .. pvalue.script)
            end
        })
        ...
        if neuronsuite.preferences.menulastselected["governor"] == pidx then neuronsuite.app.formFields[pidx]:focus() end
