...
local total_bytes = neuronsuite.preferences.developer.mspexpbytes
local fieldMap = {}
...
local function buildFieldMap()
    fieldMap = {}
    for _, field in ipairs(neuronsuite.app.Page.fields or {}) do
...
-- Init API
local apidata = generateMSPAPI(neuronsuite.preferences.developer.mspexpbytes)
...
local function postLoad()
    ...
    if neuronsuite.app.Page.apidata.receivedBytesCount['EXPERIMENTAL'] == 0 then
    ...
    if total_bytes ~= neuronsuite.app.Page.apidata.receivedBytesCount['EXPERIMENTAL'] then

        neuronsuite.preferences.developer.mspexpbytes = neuronsuite.app.Page.apidata.receivedBytesCount['EXPERIMENTAL']
...
local function wakeup()
    if not enableWakeup or not neuronsuite.app.Page or not neuronsuite.app.Page.fields then
        return
    end
    periodicSync()
end
