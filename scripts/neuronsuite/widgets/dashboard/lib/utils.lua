...
-- This function returns true if the `neuronsuite` table, its `session` field,
...
function utils.isModelPrefsReady()
    return neuronsuite and neuronsuite.session and neuronsuite.session.modelPreferences
end
...
function utils.getFontListsForResolution()
    ...
    if not radios[resolution] then
        neuronsuite.utils.log("Unsupported resolution: " .. resolution .. ". Using default fonts.","info")
...
function utils.box(
    ...
    if image then
        ...
        if type(image) == "string" and neuronsuite and neuronsuite.utils and neuronsuite.utils.loadImage then
            imageCache = imageCache or {}
            local cacheKey = image or "default_image"
            bitmapPtr = imageCache[cacheKey]
            if not bitmapPtr then
                bitmapPtr = neuronsuite.utils.loadImage(image, nil, "widgets/dashboard/gfx/logo.png")
