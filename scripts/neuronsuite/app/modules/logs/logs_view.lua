...
local graphPos = {}
graphPos['menu_offset'] = neuronsuite.app.radio.logGraphMenuOffset
graphPos['height_offset'] = neuronsuite.app.radio.logGraphHeightOffset or 0
graphPos['x_start'] = 0
graphPos['y_start'] = 0 + graphPos['menu_offset']
graphPos['width'] = math.floor(LCD_W * neuronsuite.app.radio.logGraphWidthPercentage)
...
local logColumns = neuronsuite.tasks.logging.getLogTable()
...
function readNextChunk()
    ...
    if chunk then
        ...
        neuronsuite.utils.log("Read " .. #chunk .. " bytes from log file","debug")
    else
        logFileHandle:close()
        logFileHandle = nil
        ...
        neuronsuite.utils.log("Read complete, total size: " .. #logDataRaw .. " bytes","debug")
    end
end
...
local function drawKey(name, keyunit, keyminmax, keyfloor, color, minimum, maximum, laneY, laneHeight)
    ...
    lcd.font(neuronsuite.app.radio.logKeyFont)
    ...
    lcd.font(neuronsuite.app.radio.logKeyFontSmall)
    ...
    if neuronsuite.app.radio.logShowAvg == true then
...
local function drawCurrentIndex(points, position, totalPoints, keyindex, keyunit, keyfloor, name, color, laneY, laneHeight, laneNumber, totalLanes)
    ...
    local sliderPadding = neuronsuite.app.radio.logSliderPaddingLeft
...
local function openPage(pidx, title, script, logfile, displaymode,dirname)
    ...
    neuronsuite.app.ui.fieldHeader("Logs / ".. name .. " / " .. extractShortTimestamp(logfile))
    activeLogFile = logfile

    local filePath

    if neuronsuite.app.activeLogDir then
        filePath = utils.getLogDir(neuronsuite.app.activeLogDir) .. "/" .. logfile
    else
    ...
    neuronsuite.app.formFields[1] = form.addSliderField(nil, posField, 0, 100, function()
    ...
    neuronsuite.app.formFields[2] = form.addButton(line, posField, {
        text = "-",
        icon = nil,
        options = FONT_M,
        press = function()
            if zoomLevel > 1 then
                ...
                lcd.invalidate()
                neuronsuite.app.formFields[2]:enable(true)
                neuronsuite.app.formFields[3]:enable(true)
            end    
            if zoomLevel == 1 then
                neuronsuite.app.formFields[2]:enable(false)
                neuronsuite.app.formFields[3]:focus()   
            end
        end
    })
    -- disable on start
    ...
    neuronsuite.app.formFields[3] = form.addButton(line, posField, {
        text = "+",
        icon = nil,
        options = FONT_M,
        press = function()
            ...
            if zoomLevel == zoomCount then
                ...
                neuronsuite.app.formFields[2]:focus()   
            end    
        end
    })
    
    neuronsuite.app.formFields[1]:step(1)
...
local function event(event, category, value, x, y)
    if  value == 35 then
        neuronsuite.app.ui.openPage(neuronsuite.app.lastIdx, neuronsuite.app.lastTitle, "logs/logs_logs.lua")
        return true
    end
    return false
end
...
local function wakeup()
    ...
    if logDataRawReadComplete and not processedLogData then
        ...
        if not carriedOver then
            ...
            neuronsuite.app.formNavigationFields['menu']:focus(true)
        ...
        if currentDataIndex >= #logColumns then
            ...
            local btnMinus = neuronsuite.app.formFields[2]
            local btnPlus  = neuronsuite.app.formFields[3]
            if zoomCount <= 1 then
