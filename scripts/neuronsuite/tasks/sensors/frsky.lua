...
local function telemetryActive()
  return neuronsuite and neuronsuite.session and neuronsuite.session.telemetryState == true
end

-- Detect changes to MSP-provided whitelist
local function slotsFingerprint()
  local slots = neuronsuite and neuronsuite.session and neuronsuite.session.telemetryConfig 
...
local function getSidList()
  local mod = neuronsuite and neuronsuite.tasks and neuronsuite.tasks.sensors
...
function frsky.setFblSensors(fblIds)
  ...
  if neuronsuite and neuronsuite.tasks and neuronsuite.tasks.sensors then
    neuronsuite.tasks.sensors.sid = nil
  end
  collectgarbage("collect")
end

-- Default stub (until caller provides the real profile list)
frsky.setFblSensors(neuronsuite.session.telemetryConfig)

----------------------------------------------------------------------
-- Helpers: create, rename (same flow; gated by tiny maps)
----------------------------------------------------------------------
local function createSensor(physId, primId, appId, frameValue)
  if neuronsuite.session.apiVersion == nil then return "skip" end
  ...
  if frsky.createSensorCache[appId] == nil then
    ...
    if frsky.createSensorCache[appId] == nil then
      ...
      s:appId(appId)
      s:physId(physId)
      s:module(neuronsuite.session.telemetrySensor:module())
      ...
      if v.extras then
        for _, e in ipairs(v.extras) do
          ...
          if not existing then
            ...
            sExtra:module(neuronsuite.session.telemetrySensor:module())
...
local function telemetryPop()
  if not neuronsuite.tasks.msp.sensorTlm then return false end

  local frame = neuronsuite.tasks.msp.sensorTlm:popFrame()
...
function frsky.wakeup()
  ...
  if not neuronsuite.session.telemetryState or not neuronsuite.session.telemetrySensor then
    frsky.reset()
    return
  end
  if not (neuronsuite.tasks and neuronsuite.tasks.telemetry and neuronsuite.tasks.msp and neuronsuite.tasks.msp.mspQueue) then
    return
  end
  ...
  if fp ~= _lastSlotsFp then
    _lastSlotsFp = fp
    frsky.setFblSensors(neuronsuite.session.telemetryConfig or {})
    frsky.reset() -- clears create/rename caches so next frames re-apply rules
  end  

  if neuronsuite.app and neuronsuite.app.guiIsRunning == false and neuronsuite.tasks.msp.mspQueue:isProcessed() then


    -- Only drain frames while there is work to do (create/rename).
    if telemetryActive() and neuronsuite.session.telemetrySensor and hasPendingActions() then
      local n = 0
      while telemetryPop() do
        n = n + 1
        if n >= 50 then break end
        if neuronsuite.app.triggers.mspBusy == true then break end
