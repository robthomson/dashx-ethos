...
function elrs.crossfirePop()
  ...
  if (CRSF_PAUSE_TELEMETRY == true or neuronsuite.session.mspBusy == true or not telemetryActive()) then
...
function elrs.wakeup()
  ...
  if telemetryActive() and neuronsuite.session.telemetrySensor then
    ...
    while elrs.crossfirePop() do
      ...
      if CRSF_PAUSE_TELEMETRY == true or neuronsuite.session.mspBusy == true then break end
    end
    refreshStaleSensors()
  else
    resetSensors()
