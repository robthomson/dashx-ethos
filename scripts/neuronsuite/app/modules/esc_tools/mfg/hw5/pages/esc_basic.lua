...
function postLoad()
    ...
    if neuronsuite.app.Page.apidata and neuronsuite.app.Page.apidata.other  and neuronsuite.app.Page.apidata.other['ESC_PARAMETERS_HW5'] then
        local version
        if neuronsuite.session.escDetails and neuronsuite.session.escDetails.version then
            version = neuronsuite.session.escDetails.version
        else
            version = "default"
        end

        if neuronsuite.app.Page.apidata.other['ESC_PARAMETERS_HW5'][version] then
            local newVoltage = neuronsuite.app.Page.apidata.other['ESC_PARAMETERS_HW5'][version]

            local voltageTable = neuronsuite.app.utils.convertPageValueTable(newVoltage, -1)   
    
            -- we target field 3 as we know it is bec_voltage
            neuronsuite.app.formFields[3]:values(voltageTable)
        end
...
local function onNavMenu(self)
    neuronsuite.app.triggers.escToolEnableButtons = true
    neuronsuite.app.ui.openPage(pidx, folder, "esc_tools/esc_tool.lua")
end
...
return {
    ...
    headerLine = neuronsuite.escHeaderLineText
}
