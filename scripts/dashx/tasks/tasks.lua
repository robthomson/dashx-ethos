...
function tasks.telemetryCheckScheduler()
    ...
    if system.getVersion().simulation and dashx.simevent.telemetry_state == false then telemetryState = false end
...
function tasks.active()
    ...
    tasks.wasOn = age >= 2
    if dashx.app.triggers and dashx.app.triggers.mspBusy or age <= 2 then return true end
...
local function canRunTask(task, now)
    ...
    local ok = linkOK and connOK and (priorityTask or od >= 0 or not (dashx.app.triggers and dashx.app.triggers.mspBusy)) and (not task.simulatoronly or usingSimulator)
    ...
    do
        ...
        if (now2 - last_mem_t) >= MEM_PERIOD then
            ...
            if m and m.luaRamAvailable then
                ...
                dashx.session.freeram = mem_avg_kb
