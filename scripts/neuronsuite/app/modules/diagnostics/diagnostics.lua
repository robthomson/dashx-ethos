...
local function openPage(pidx, title, script)
    ...
    if neuronsuite.preferences.menulastselected["diagnostics"] == nil then neuronsuite.preferences.menulastselected["diagnostics"] = 1 end
    ...
    for pidx, pvalue in ipairs(S_PAGES) do
        ...
        if lc == 0 then
            if neuronsuite.preferences.general.iconsize == 0 then y = form.height() + app.radio.buttonPaddingSmall end
            if neuronsuite.preferences.general.iconsize == 1 then y = form.height() + app.radio.buttonPaddingSmall end
            if neuronsuite.preferences.general.iconsize == 2 then y = form.height() + app.radio.buttonPadding end
        end
        ...
        app.formFields[pidx] = form.addButton(line, {x = bx, y = y, w = buttonW, h = buttonH}, {
            ...
            press = function()
                neuronsuite.preferences.menulastselected["diagnostics"] = pidx
                app.ui.progressDisplay(nil,nil,true)
                app.ui.openPage(pidx, neuronsuite.i18n.get("app.modules.diagnostics.name")  .. " / " .. pvalue.name, "diagnostics/tools/" .. pvalue.script)
            end
        })

        if neuronsuite.preferences.menulastselected["diagnostics"] == pidx then app.formFields[pidx]:focus() end
