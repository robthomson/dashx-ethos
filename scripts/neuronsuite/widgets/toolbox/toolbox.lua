...
function toolbox.paint(widget)
    ...
    if not neuronsuite.session.toolbox then
        return
    end
    ...
    local msg = neuronsuite.session.toolbox[toolBoxList[widget.object].object] or "-"
...
function toolbox.close()
    neuronsuite.session.toolbox = nil
end
