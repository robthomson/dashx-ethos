...
function tasks.isTaskActive(name)
    for _, t in ipairs(tasksList) do
        if t.name == name then
            ...
            if name == "msp" then
                return neuronsuite.session.mspBusy
...
function tasks.active()
    ...
    tasks.wasOn = age >= 2
    if neuronsuite.session.mspBusy or age <= 2 then return true end
...
local function canRunTask(task, now)
    ...
    local ok =
        linkOK
        and connOK
        and (priorityTask or od >= 0 or not neuronsuite.session.mspBusy)
