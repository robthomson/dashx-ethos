...
local function checkLogdirExists()
        ...
        os.mkdir("LOGS:/neuronsuite/telemetry")
        os.mkdir("LOGS:/neuronsuite/telemetry/" .. neuronsuite.session.mcu_id)
end
...
function logging.writeLogs(forcewrite)
    ...
    if #log_queue > 0 and logFileName then
        local filePath = "LOGS:neuronsuite/telemetry/" .. neuronsuite.session.mcu_id .. "/" .. logFileName
...
function logging.getLogHeader()
    ...
    return "time, " .. neuronsuite.utils.joinTableItems(names, ", ")
end
...
function logging.getLogLine()
    ...
    local ts = os.time()
    return ts .. ", " .. neuronsuite.utils.joinTableItems(values, ", ")
end
...
function logging.flushLogs()
    if logFileName or logHeader then
        neuronsuite.utils.log("Flushing logs - " .. tostring(logFileName), "info")
...
function logging.wakeup()
    ...
    if neuronsuite.utils.inFlight() then
        if not logFileName then
            logFileName = generateLogFilename()
            neuronsuite.utils.log("Logging triggered by inFlight() - " .. logFileName, "info")

            local iniName = "LOGS:neuronsuite/telemetry/" .. neuronsuite.session.mcu_id .. "/logs.ini"
            local iniData = neuronsuite.ini.load_ini_file(iniName) or {}
            if not iniData.model then
                iniData.model = {}
            end
            iniData.model.name = neuronsuite.session.craftName or model.name() or "Unknown"
            neuronsuite.ini.save_ini_file(iniName, iniData)
        end
