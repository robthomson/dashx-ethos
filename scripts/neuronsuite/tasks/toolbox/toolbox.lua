...
for _, name in ipairs(taskNames) do
    toolbox[name] = assert(neuronsuite.compiler.loadfile("tasks/toolbox/tasks/" .. name .. ".lua"))(neuronsuite.config)
    table.insert(wakeupHandlers, function() toolbox[name].wakeup() end)
end

function toolbox.wakeup()

    if neuronsuite.session.toolbox == nil then
        return
    end
    local currentTime = os.clock()

    if neuronsuite.session.isConnected and neuronsuite.session.telemetryState then
