...
function utils.getRSSI()
    if neuronsuite.simevent.rflink == 1 then
        return 0
    end

    if neuronsuite.app.offlineMode == true then
        return 100
    end

    if neuronsuite.session.telemetryState then
        return 100
    else
        return 0
    end
...
function utils.getFieldValue(f)
    ...
    if f.decimals then
        v = neuronsuite.utils.round(v * neuronsuite.app.utils.decimalInc(f.decimals), 2)
    end
...
--[[
    ...
    @param f (table) Field to save to. Optional keys:
        ...
        - postEdit (func)   Called as postEdit(neuronsuite.app.Page) after save.
...
function utils.saveFieldValue(f, value)
    if value then
        ...
        if f.decimals then
            f.value = value / neuronsuite.app.utils.decimalInc(f.decimals)
        else
            f.value = value
        end
...
function utils.scaleValue(value, f)
    ...
    local v = value * neuronsuite.app.utils.decimalInc(f.decimals)
    if f.scale then v = v / f.scale end
    return neuronsuite.utils.round(v)
end
...
function utils.getInlinePositions(f, lPage)
    ...
    local inline_size = utils.getInlineSize(f.label, lPage) * neuronsuite.app.radio.inlinesize_mult
    ...
    local eH      = neuronsuite.app.radio.navbuttonHeight
    local eY      = neuronsuite.app.radio.linePaddingTop
...
function utils.getCurrentProfile()
    local pidProfile  = neuronsuite.tasks.telemetry.getSensor("pid_profile")
    local rateProfile = neuronsuite.tasks.telemetry.getSensor("rate_profile")

    if (pidProfile ~= nil and rateProfile ~= nil) then
        neuronsuite.session.activeProfileLast = neuronsuite.session.activeProfile
        local p = pidProfile
        if p ~= nil then
            neuronsuite.session.activeProfile = math.floor(p)
        else
            neuronsuite.session.activeProfile = nil
        end

        neuronsuite.session.activeRateProfileLast = neuronsuite.session.activeRateProfile
        local r = rateProfile
        if r ~= nil then
            neuronsuite.session.activeRateProfile = math.floor(r)
        else
