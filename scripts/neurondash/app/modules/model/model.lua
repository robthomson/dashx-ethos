...
local function openPage(pidx, title, script)
    ...
    if neurondash.app.gfx_buttons["model"] == nil then neurondash.app.gfx_buttons["model"] = {} end
    if neurondash.preferences.menulastselected["model"] == nil then neurondash.preferences.menulastselected["model"] = 1 end
    ...
    for pidx, pvalue in ipairs(S_PAGES) do
        ...
        if neurondash.preferences.general.iconsize ~= 0 then
            if neurondash.app.gfx_buttons["model"][pidx] == nil then neurondash.app.gfx_buttons["model"][pidx] = lcd.loadMask("app/modules/model/gfx/" .. pvalue.image) end
        else
            neurondash.app.gfx_buttons["model"][pidx] = nil
        end

        neurondash.app.formFields[pidx] = form.addButton(line, {x = bx, y = y, w = buttonW, h = buttonH}, {
            text = pvalue.name,
            icon = neurondash.app.gfx_buttons["model"][pidx],
            options = FONT_S,
            paint = function()
            end,
            press = function()
                neurondash.preferences.menulastselected["model"] = pidx
                neurondash.app.ui.progressDisplay()
                neurondash.app.ui.openPage(pidx, pvalue.folder, "model/tools/" .. pvalue.script)
            end
        })

        if pvalue.disabled == true then neurondash.app.formFields[pidx]:enable(false) end

        if neurondash.preferences.menulastselected["model"] == pidx then neurondash.app.formFields[pidx]:focus() end
