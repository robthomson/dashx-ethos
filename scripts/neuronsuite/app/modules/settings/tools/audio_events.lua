...
local function openPage(pageIdx, title, script)
    ...
    local eventList = neuronsuite.tasks.events.telemetry.eventTable
    local eventNames = sensorNameMap(neuronsuite.tasks.telemetry.listSensors())

    -- Prepare working config as a shallow copy of events preferences
    local savedEvents = neuronsuite.preferences.events or {}
    ...
    neuronsuite.app.formFields[formFieldCount] = form.addBooleanField(
        ...
        function(val)
            config.temp_esc = val
            setFieldEnabled(neuronsuite.app.formFields[escFields.thresh], val)
        end
    )
    ...
    neuronsuite.app.formFields[formFieldCount] = form.addNumberField(
        escThresh, nil, 60, 300,
        function() return config.escalertvalue or 90 end,
        function(val) config.escalertvalue = val end,
        1
    )
    neuronsuite.app.formFields[formFieldCount]:suffix("°")
    setFieldEnabled(neuronsuite.app.formFields[escFields.thresh], escEnabled)
    ...
    neuronsuite.app.formFields[formFieldCount] = form.addBooleanField(
        ...
        function(val)
            ...
            setFieldEnabled(neuronsuite.app.formFields[fuelFields.callout], val)
            setFieldEnabled(neuronsuite.app.formFields[fuelFields.repeats], val)
            setFieldEnabled(neuronsuite.app.formFields[fuelFields.haptic], val)
        end
    )
    ...
    setFieldEnabled(neuronsuite.app.formFields[fuelFields.callout], fuelEnabled)
    ...
    neuronsuite.app.formFields[formFieldCount]:suffix("x")
    setFieldEnabled(neuronsuite.app.formFields[fuelFields.repeats], fuelEnabled)
    ...
    setFieldEnabled(neuronsuite.app.formFields[fuelFields.haptic], fuelEnabled)

    setFieldEnabled(neuronsuite.app.formFields[escFields.enable], true)
    setFieldEnabled(neuronsuite.app.formFields[becFields.enable], true)
    setFieldEnabled(neuronsuite.app.formFields[fuelFields.enable], true)
    ...
    local buttons = {
        {
            ...
            action = function()
                ...
                for key, value in pairs(config) do
                    neuronsuite.preferences.events[key] = value
                end
