...
local function openPage(idx, title, script, source, folder,themeScript)
    ...
    neuronsuite.app.dashboardEditingTheme = source .. "/" .. folder

    -- Load the module
    local modulePath =  themeScript

    page = assert(neuronsuite.compiler.loadfile(modulePath))(idx)
    ...
    neuronsuite.app.formNavigationFields['save'] = form.addButton(line, {x = x, y = neuronsuite.app.radio.linePaddingTop, w = buttonW, h = neuronsuite.app.radio.navbuttonHeight}, {
        text = "SAVE",
        icon = nil,
        options = FONT_S,
        paint = function()
