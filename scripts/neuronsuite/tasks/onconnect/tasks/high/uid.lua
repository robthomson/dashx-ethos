...
function uid.wakeup()
    ...
    if (neuronsuite.session.mcu_id == nil and mspCallMade == false)  then


        mspCallMade = true

        local API = neuronsuite.tasks.msp.api.load("UID")
        API.setCompleteHandler(function(self, buf)
            ...
            if U_ID_0 and U_ID_1 and U_ID_2 then
                ...
                if uid then
                    neuronsuite.utils.log("MCU ID: " .. uid, "info")
                end
                neuronsuite.session.mcu_id = uid
            end
...
function uid.reset()
    neuronsuite.session.mcu_id = nil
    mspCallMade = false
end

function uid.isComplete()
    if neuronsuite.session.mcu_id ~= nil  then
        return true
    end
end
