...
local compiler = neuronsuite.compiler.loadfile
...
function tasks.isTaskActive(name)
    for _, t in ipairs(tasksList) do
        if t.name == name then
            ...
            if name == "msp" then
                return neuronsuite.app.triggers.mspBusy
...
function tasks.initialize()
    ...
    if io.open(cachePath, "r") then
        local ok, cached = pcall(neuronsuite.compiler.dofile, cachePath)
...
local function clearSessionAndQueue()
    ...
    utils.session()
    local q = neuronsuite.tasks and neuronsuite.tasks.msp and neuronsuite.tasks.msp.mspQueue
...
function tasks.telemetryCheckScheduler()
    ...
    if system.getVersion().simulation and neuronsuite.simevent.telemetry_state == false then
    ...
    if currentSensor then
       neuronsuite.session.telemetryState  = true
       neuronsuite.session.telemetrySensor = currentSensor
       neuronsuite.session.telemetryModule = currentModuleId
       neuronsuite.session.telemetryType   = currentTelemetryType 
    ...
    if currentTelemetryType ~= lastTelemetryType then
        neuronsuite.utils.log("Telemetry type changed to " .. tostring(currentTelemetryType), "info")
        tasks.setTelemetryTypeChanged()
...
function tasks.active()
    ...
    tasks.wasOn = age >= 2
    if neuronsuite.app.triggers.mspBusy or age <= 2 then return true end
...
local function canRunTask(task, now)
    ...
    local linkOK = not task.linkrequired or neuronsuite.session.telemetryState
    local connOK = not task.connected    or neuronsuite.session.isConnected

    local ok =
        linkOK
        and connOK
        and (priorityTask or od >= 0 or not neuronsuite.app.triggers.mspBusy)
...
function tasks.wakeup()
    ...
    tasks.profile.enabled = neuronsuite.preferences and neuronsuite.preferences.developer and neuronsuite.preferences.developer.taskprofiler
  ...
  neuronsuite.session.cpuload = math.min(100, math.max(0, cpu_avg * 100))
  ...
  last_wakeup_start = now
    ...
    if (now2 - last_mem_t) >= MEM_PERIOD then
        ...
        neuronsuite.session.freeram   = mem_avg_kb          -- KB (ema)
        neuronsuite.session.luaUsedKb = collectgarbage and collectgarbage("count") or nil
        neuronsuite.session.memSource = "system"
        else
        ...
        neuronsuite.session.luaUsedKb = used_kb
        -- keep last known free if we had one; otherwise mark as nil
        neuronsuite.session.freeram   = mem_avg_kb          -- may be nil if never known
        neuronsuite.session.memSource = "lua"
        end
    end
    end
...
-- After updating all tasks, invokes the `neuronsuite.utils.session()` function.
function tasks.setTelemetryTypeChanged()
    for _, task in ipairs(tasksList) do
        if tasks[task.name].setTelemetryTypeChanged then
            --neuronsuite.utils.log("Notifying task [" .. task.name .. "] of telemetry type change", "info")
