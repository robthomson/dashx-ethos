...
function craftimage.wakeup()
    if neuronsuite.session.toolbox.craftimage ~= nil then
        -- prevent multiple image loads
        return
    end

    local craftName = neuronsuite.session.craftName
    --local modelID = neuronsuite.session.modelID
    ...
    bitmapPtr = neuronsuite.utils.loadImage(image1, image2, image3, image4)
    ...
    if not bitmapPtr then
        bitmapPtr = neuronsuite.utils.loadImage(default_image)
    end

    neuronsuite.session.toolbox.craftimage = bitmapPtr
end
