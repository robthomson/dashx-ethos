...
local renders = neuronsuite.widgets.dashboard.renders
local folder = "SCRIPTS:/" .. neuronsuite.config.baseDir .. "/widgets/dashboard/objects/navigation/"
...
function wrapper.wakeup(box)
    ...
    if not renders[subtype] then
        ...
        local loader = neuronsuite.compiler.loadfile(path)
        if loader then
