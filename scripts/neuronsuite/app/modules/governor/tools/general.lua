...
local apidata = {
        ...
        formdata = {
            ...
            {t = i18n("app.modules.profile_governor.idle_throttle"),  mspapi = 1, apikey = "governor_idle_throttle", enablefunction = function() return (neuronsuite.session.governorMode >=1 ) end},
            {t = i18n("app.modules.profile_governor.auto_throttle"),  mspapi = 1, apikey = "governor_auto_throttle", enablefunction = function() return (neuronsuite.session.governorMode >=1 ) end},
...
local function disableFields()
    for i, _ in ipairs(neuronsuite.app.formFields) do

        if type(neuronsuite.app.formFields[i]) == "userdata" then
            if i ~= 1 and neuronsuite.app.formFields[i] then
...
local function enableFields()
    for i, _ in ipairs(neuronsuite.app.formFields) do
        if type(neuronsuite.app.formFields[i]) == "userdata" then
            if i ~= 1 and neuronsuite.app.formFields[i] then
               neuronsuite.app.formFields[i]:enable(true)
            end
        end
    end
end
...
local function setGovernorMode(self)
    local currentIndex = math.floor(neuronsuite.app.Page.fields[1].value)
...
local function postSave(self)
    ...
    if neuronsuite.session.governorMode ~= neuronsuite.app.Page.fields[1].value then
        neuronsuite.session.governorMode = neuronsuite.app.Page.fields[1].value
        neuronsuite.utils.log("Governor mode: " .. neuronsuite.session.governorMode,"info")
    end
    return payload
end
