...
function utils.resolveModelName(foldername)
    ...
    local iniName = "LOGS:neuronsuite/telemetry/" .. foldername .. "/logs.ini"
...
function utils.getLogPath()
    ...
    if neuronsuite.app.activeLogDir then
        return string.format("LOGS:/neuronsuite/telemetry/%s/", neuronsuite.app.activeLogDir)
    end
    return "LOGS:/neuronsuite/telemetry/"
end
...
function utils.getLogDir(dirname)
    ...
    if not dirname then
        local defaultDir = "LOGS:/neuronsuite/telemetry/" .. neuronsuite.session.mcu_id .. "/"
        os.mkdir(defaultDir)
        return defaultDir
    end

    -- Return requested directory
    return "LOGS:/neuronsuite/telemetry/" .. dirname .. "/" 
end
