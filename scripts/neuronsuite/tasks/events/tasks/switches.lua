...
-- 
-- This function retrieves the user's switch preferences from `neuronsuite.preferences.switches`.
...
-- @usage
--   initializeSwitches()
-- @see neuronsuite.preferences.switches
-- @see system.getSource
-- @see neuronsuite.tasks.telemetry.listSensorAudioUnits
local function initializeSwitches()
    local prefs = neuronsuite.preferences.switches
    ...
    switchTable.units = neuronsuite.tasks.telemetry.listSensorAudioUnits()
end
...
--   - `neuronsuite.tasks.telemetry.getSensorSource(key)`: Retrieves the sensor source for a switch.
...
function switches.wakeup()
    ...
    local mode = (neuronsuite and neuronsuite.flightmode and neuronsuite.flightmode.current) or nil
    ...
    for key, sensor in pairs(switchTable.switches) do
        ...
        if playNow then
            local sensorSrc = neuronsuite.tasks.telemetry.getSensorSource(key)
            if sensorSrc then
