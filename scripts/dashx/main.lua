...
config.toolName = "DashX"                                                     -- name of the tool 
...
config.baseDir = "dashx"                                                          -- base directory for the suite. This is only used by msp api to ensure correct path
...
--[[
    ...
    6. Assigns the merged preferences to `dashx.preferences`.
...
dashx.ini = assert(loadfile("lib/ini.lua"))(config) -- self contantained and never compiled
...
-- tasks
dashx.config.bgTaskName = dashx.config.toolName .. " [Background]"              -- background task name for msp services etc
dashx.config.bgTaskKey = "dshxbg"                                          -- key id used for msp services

-- main
-- dashx: Main table for the dashx script.
-- dashx.config: Configuration table for the suite.
-- dashx.session: Session table for the suite.
-- dashx.app: Application module loaded from "app/app.lua" with the provided configuration.
dashx.compiler = assert(loadfile("lib/compile.lua"))(dashx.config) 
...
-- 
-- This script initializes the `dashx` tasks and background task.
-- 
-- The `dashx.tasks` table is created to hold various tasks.
-- The `dashx.tasks` is assigned the result of executing the "tasks/tasks.lua" file with the `config` parameter.
-- The `dashx.compiler.loadfile` function is used to load the "tasks/tasks.lua" file, and `assert` ensures that the file is loaded successfully.
-- The loaded file is then executed with the `config` parameter, and its return value is assigned to `dashx.tasks`.
-- tasks
...
function dashx.version()
    local version = dashx.config.version.major .. "." .. dashx.config.version.minor .. "." .. dashx.config.version.revision .. "-" .. dashx.config.version.suffix
    return {
        version = version,
        major = dashx.config.version.major,
        minor = dashx.config.version.minor,
        revision = dashx.config.version.revision,
        suffix = dashx.config.version.suffix
    }
end


--[[
    Initializes the main script for the dashx.

    This function performs the following tasks:
    1. Checks if the Ethos version is supported using `dashx.utils.ethosVersionAtLeast()`.
    ...
    4. Dynamically loads and registers widgets:
       - Finds widget scripts using `dashx.utils.findWidgets()`.
       - Loads each widget script dynamically using `dashx.compiler.loadfile()`.
       - Assigns the loaded script to a variable inside the `dashx` table.
    ...
    Dependencies:
    - `dashx.utils.ethosVersionAtLeast()`
    - `system.registerSystemTool()`
    - `system.registerTask()`
    - `dashx.utils.findWidgets()`
    - `dashx.compiler.loadfile()`
...
local function init()
 ...
 --       create = dashx.app.create_logtool,
 --       wakeup = dashx.app.wakeup,
 --       paint = dashx.app.paint,
 --       close = dashx.app.close
 --   })
    ...
    -- 2. Loads the script file from the specified folder and assigns it to a variable inside the dashx table.
    -- 3. Uses the script name (or a provided variable name) as a key to store the loaded script module in the dashx table.
    ...
    --   - varname (optional): A custom variable name to use for storing the script module in the dashx table.
    -- 
    ...
    dashx.widgets = {}
        ...
        for i, v in ipairs(widgetList) do
            if v.script then
                ...
                -- Store the module inside dashx.widgets
