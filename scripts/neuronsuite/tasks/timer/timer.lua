...
function timer.reset()
    --neuronsuite.utils.log("Resetting flight timers", "info")
    lastFlightMode = nil

    local timerSession = {}
    neuronsuite.session.timer = timerSession
    neuronsuite.session.flightCounted = false

    timerSession.baseLifetime = tonumber(
        neuronsuite.ini.getvalue(neuronsuite.session.modelPreferences, "general", "totalflighttime")
    ) or 0
...
function timer.save()
    local prefs = neuronsuite.session.modelPreferences
    local prefsFile = neuronsuite.session.modelPreferencesFile

    if not prefsFile then
        neuronsuite.utils.log("No model preferences file set, cannot save flight timers", "info")
        return 
    end

    neuronsuite.utils.log("Saving flight timers to INI: " .. prefsFile, "info")

    if prefs then
        neuronsuite.ini.setvalue(prefs, "general", "totalflighttime", neuronsuite.session.timer.baseLifetime or 0)
        neuronsuite.ini.setvalue(prefs, "general", "lastflighttime", neuronsuite.session.timer.session or 0)
        neuronsuite.ini.save_ini_file(prefsFile, prefs)
    end    
end
...
local function finalizeFlightSegment(now)
    local timerSession = neuronsuite.session.timer
    local prefs = neuronsuite.session.modelPreferences
    ...
    if timerSession.baseLifetime == nil then
        timerSession.baseLifetime = tonumber(
            neuronsuite.ini.getvalue(prefs, "general", "totalflighttime")
        ) or 0
    end
    ...
    if prefs then
        neuronsuite.ini.setvalue(prefs, "general", "totalflighttime", timerSession.baseLifetime)
    end
    timer.save()
end
...
--
-- Dependencies:
--   - Relies on `neuronsuite.session` for session state.
--   - Uses `neuronsuite.ini` for reading and writing model preferences.
--   - Calls `finalizeFlightSegment(now)` when appropriate.
function timer.wakeup()
    local now = os.time()
    local timerSession = neuronsuite.session.timer
    local prefs = neuronsuite.session.modelPreferences
    local flightMode = neuronsuite.flightmode.current
    ...
    if flightMode == "inflight" then
        ...
        if timerSession.live >= 25 and not neuronsuite.session.flightCounted then
            neuronsuite.session.flightCounted = true

            if prefs and neuronsuite.ini.section_exists(prefs, "general") then
                local count = neuronsuite.ini.getvalue(prefs, "general", "flightcount") or 0
                neuronsuite.ini.setvalue(prefs, "general", "flightcount", count + 1)
                neuronsuite.ini.save_ini_file(neuronsuite.session.modelPreferencesFile, prefs)
            end
        end
