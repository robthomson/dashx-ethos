...
neuronsuite.session.dialImageCache = neuronsuite.session.dialImageCache or {}
local function loadDialPanelCached(dialId)
    local key = tostring(dialId or "panel1")
    if not neuronsuite.session.dialImageCache[key] then
        local panelPath = resolveDialAsset(dialId, "widgets/dashboard/gfx/dials") or "widgets/dashboard/gfx/dials/panel1.png"
        neuronsuite.session.dialImageCache[key] = neuronsuite.utils.loadImage(panelPath)
    end
    return neuronsuite.session.dialImageCache[key]
end
