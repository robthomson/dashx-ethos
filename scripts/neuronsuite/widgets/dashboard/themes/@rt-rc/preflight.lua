...
local utils = neuronsuite.widgets.dashboard.utils
...
local function getUserVoltageOverride(which)
  local prefs = neuronsuite.session and neuronsuite.session.modelPreferences
...
local function getThemeValue(key)
    ...
    if key == "tx_min" or key == "tx_warn" or key == "tx_max" then
        if neuronsuite and neuronsuite.preferences and neuronsuite.preferences.general then
            local val = neuronsuite.preferences.general[key]
            if val ~= nil then return tonumber(val) end
        end
    end
    -- Theme defaults for other values
    if neuronsuite and neuronsuite.session and neuronsuite.session.modelPreferences and neuronsuite.session.modelPreferences[theme_section] then
        local val = neuronsuite.session.modelPreferences[theme_section][key]
        val = tonumber(val)
...
local function header_boxes()
    local txbatt_type = 0
    if neuronsuite and neuronsuite.preferences and neuronsuite.preferences.general then
        txbatt_type = neuronsuite.preferences.general.txbatt_type or 0
    end
    ...
    return{
      ...
      },
      {
        ...
        min = function()
            ...
            local cfg = neuronsuite.session.batteryConfig
...
local function boxes()
    local config = neuronsuite and neuronsuite.session and neuronsuite.session.modelPreferences and neuronsuite.session.modelPreferences[theme_section]
