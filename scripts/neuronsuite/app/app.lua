...
local function rebootFc()
  ...
  neuronsuite.tasks.msp.mspQueue:add({
...
function app.settingsSaved()
  if app.Page and app.Page.eepromWrite then
    if app.pageState ~= app.pageStatus.eepromWrite then
      ...
      app.triggers.closeSave = true
      if neuronsuite.session.isArmed then
...
local function saveSettings()
  ...
  neuronsuite.app.Page.apidata.apiState.isProcessing = true
  ...
  for apiID, apiNAME in ipairs(apiList) do
    ...
    local API = neuronsuite.tasks.msp.api.load(apiNAME)
...
local function requestPage()
  ...
  local function processNextAPI()
    ...
    if not apiKey then
      ...
      neuronsuite.tasks.callback.inSeconds(backoff + jitter, processNextAPI)
      return
    end

    local API = neuronsuite.tasks.msp.api.load(v)
    ...
    local function handleTimeout()
      ...
      if retryCount < 3 then
        ...
        neuronsuite.tasks.callback.inSeconds(0.25, processNextAPI)
      else
    ...
    neuronsuite.tasks.callback.inSeconds(2, handleTimeout)
    ...
    API.setCompleteHandler(function(self, buf)
      ...
      state.currentIndex = state.currentIndex + 1
      neuronsuite.tasks.callback.inSeconds(0.5, processNextAPI)
    end)
...
function app.updateTelemetryState()
  if system:getVersion().simulation ~= true then
    if not neuronsuite.session.telemetrySensor then
...
app._uiTasks = {
  ...
  function()
    if not (app.Page and (app.Page.refreshOnProfileChange or app.Page.refreshOnRateChange or app.Page.refreshFullOnProfileChange or app.Page.refreshFullOnRateChange)
           ...
           and neuronsuite.tasks.msp.mspQueue:isProcessed()) then return end
    local now = os.clock();
    local interval = (neuronsuite.tasks.telemetry.getSensorSource("pid_profile") and neuronsuite.tasks.telemetry.getSensorSource("rate_profile"))
                     and 0.1 or 1.5
    if (now - (app.profileCheckScheduler or 0)) >= interval then
      app.profileCheckScheduler = now
      app.utils.getCurrentProfile()
      if neuronsuite.session.activeProfileLast and app.Page.refreshOnProfileChange and
         neuronsuite.session.activeProfile ~= neuronsuite.session.activeProfileLast then
        app.triggers.reload = not app.Page.refreshFullOnProfileChange
        app.triggers.reloadFull = app.Page.refreshFullOnProfileChange
        return
      end
      if neuronsuite.session.activeRateProfileLast and app.Page.refreshOnRateChange and
         neuronsuite.session.activeRateProfile ~= neuronsuite.session.activeRateProfileLast then
  ...
  function()
    ...
    if app.uiState == app.uiStatus.mainMenu then
      ...
      elseif not neuronsuite.session.isConnected then
      ...
      elseif neuronsuite.session.apiVersion and neuronsuite.utils.stringInArray(neuronsuite.config.supportedMspApiVersion, apiV) then
        app.offlineMode = false
    ...
    elseif not app.isOfflinePage then
      if not neuronsuite.session.isConnected then app.ui.openMainMenu() end
    end
  end,
  ...
  function()
    ...
    if app.triggers.isSaving then
      if app.pageState >= app.pageStatus.saving and not app.dialogs.saveDisplay then
        ...
        neuronsuite.tasks.msp.mspQueue.retryCount = 0
      end
    end
  end,
  ...
  function()
    ...
    if not app.dialogs.progressDisplay then
      ...
      app.dialogs.progressCounter = 0
      local key = (neuronsuite.utils.apiVersionCompare(">=", "12.08") and "app.msg_please_disarm_to_save_warning" or "app.msg_please_disarm_to_save")
  ...
  function()
    ...
    if app.uiState == app.uiStatus.mainMenu then
      invalidatePages()
    elseif app.triggers.isReady and (neuronsuite.tasks and neuronsuite.tasks.msp and neuronsuite.tasks.msp.mspQueue:isProcessed())
...
function app.event(widget, category, value, x, y)
  ...
  if neuronsuite.app.lastMenu ~= nil and category == 3 and value == 0 then
    app.ui.openMainMenu()
    return true
  end
...
-- Close and cleanup
function app.close()
  neuronsuite.utils.reportMemoryUsage("closing application: start")

  -- Save user preferences
  local userpref_file = "SCRIPTS:/" .. neuronsuite.config.preferences .. "/preferences.ini"
  neuronsuite.ini.save_ini_file(userpref_file, neuronsuite.preferences)
  ...
  neuronsuite.config.useCompiler    = true
  ...
  neuronsuite.session.activeProfileLast = nil
  neuronsuite.session.activeProfile = nil
  neuronsuite.session.activeRateProfile = nil
  neuronsuite.session.activeRateProfileLast = nil
  neuronsuite.session.activeRateTable = nil

  collectgarbage()
  invalidatePages()

  neuronsuite.utils.reportMemoryUsage("closing application: end")
  system.exit()
  return true
end
