...
function flightmode.inFlight()
    local telemetry = neuronsuite.tasks.telemetry

    if not telemetry.active() or not neuronsuite.session.isArmed then
        return false
    end
    ...
    local rx = neuronsuite.session.rx
...
local function determineMode()
    local armed     = neuronsuite.session.isArmed
    local connected = neuronsuite.session.isConnected
    local current   = neuronsuite.flightmode.current
...
function flightmode.wakeup()
    ...
    if lastFlightMode ~= mode then
        neuronsuite.utils.log("Flight mode: " .. mode, "info")
        neuronsuite.flightmode.current = mode
        lastFlightMode = mode
    end    
end
