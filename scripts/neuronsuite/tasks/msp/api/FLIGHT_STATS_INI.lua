...
local INI_FILE = "SCRIPTS:/" .. neuronsuite.config.preferences .. "/models/" .. neuronsuite.session.mcu_id .. ".ini"
local INI_SECTION = "general"

local ini       = neuronsuite.ini
local mspModule = neuronsuite.tasks.msp.api
...
local function write()
    ...
    for k, v in pairs(payloadData) do
        ...
        -- update session data
        if neuronsuite.session.modelPreferences and neuronsuite.session.modelPreferences[INI_SECTION] then
            neuronsuite.session.modelPreferences[INI_SECTION][k] = v
        end
    end
