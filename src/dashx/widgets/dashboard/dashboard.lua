...
function dashboard.computeOverlayMessage()
    ...
    if not dashx.session.isConnectedHigh and state ~= "postflight" then return "@i18n(widgets.dashboard.waiting_for_connection)@" end
...
function dashboard.renderLayout(widget, config)
    ...
    if layout.showstats or dashx.preferences.developer.overlaystats then
        ...
        local budgetMs = (dashx.performance and dashx.performance.budget_ms) or 50
        local tickMs = (dashx.performance and dashx.performance.tick_ms)
        ...
        local schedRows = {{"LOAD", fmtPct(cpuUsage), "%"}, {"LOAD (100ms window)", fmtPct(dashx.performance.cpuload_window100 or 0), "%"}, {"HEADROOM", fmtPct(headroomPct), "%"}, {"LOOP / BUDGET", fmtMS(loopMs) .. " / " .. fmtMS(budgetMs), "ms"}}
...
function dashboard.create()
    ...
    if not dashboard.utils then dashboard.utils = assert(compile("SCRIPTS:/" .. dashx.config.baseDir .. "/widgets/dashboard/lib/utils.lua"))() end
    if not dashboard.loaders then dashboard.loaders = assert(compile("SCRIPTS:/" .. dashx.config.baseDir .. "/widgets/dashboard/lib/loaders.lua"))() end
...
function dashboard.wakeup(widget)
    ...
    if firstWakeupCustomTheme and dashx.session.mcu_id and dashx.session.modelPreferences and dashx.session.modelPreferences.dashboard then
...
function dashboard.savePreference(key, value)
    if not dashx.session.modelPreferences or not dashx.session.modelPreferencesFile or not dashboard.currentWidgetPath then return false end
    if not dashx.app.guiIsRunning then
