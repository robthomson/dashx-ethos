...
local msp = assert(neuronsuite.compiler.loadfile("tasks/sensors/msp.lua"))(config)
local smart = assert(neuronsuite.compiler.loadfile("tasks/sensors/smart.lua"))(config)
local log = neuronsuite.utils.log
...
--[[
    ...
    This function checks if the neuronsuite tasks are active and if the API version is available. 
...
local function loadSensorModule()
    ...
    if not neuronsuite.session.apiVersion then return nil end
    ...
    if system:getVersion().simulation == true then
        if not loadedSensorModule or loadedSensorModule.name ~= "sim" then
            ...
            loadedSensorModule = {name = "sim", module = assert(neuronsuite.compiler.loadfile("tasks/sensors/sim.lua"))(config)}
        end   
    elseif protocol == "crsf" then
        if not loadedSensorModule or loadedSensorModule.name ~= "elrs" then
            ...
            loadedSensorModule = {name = "elrs", module = assert(neuronsuite.compiler.loadfile("tasks/sensors/elrs.lua"))(config)}
        end
    elseif protocol == "sport" then
        if neuronsuite.utils.apiVersionCompare(">=", "12.08") then
            if not loadedSensorModule or loadedSensorModule.name ~= "frsky" then
                ...
                loadedSensorModule = {name = "frsky", module = assert(neuronsuite.compiler.loadfile("tasks/sensors/frsky.lua"))(config)}
            end
        else
            if not loadedSensorModule or loadedSensorModule.name ~= "frsky_legacy" then
                --log("Loading FrSky Legacy sensor module","info")
                loadedSensorModule = {name = "frsky_legacy", module = assert(neuronsuite.compiler.loadfile("tasks/sensors/frsky_legacy.lua"))(config)}
            end
        end
    else
...
function sensors.wakeup()
    ...
    if neuronsuite.session.resetSensors and not delayPending then
        delayStartTime = os.clock()
        delayPending = true
        neuronsuite.session.resetSensors = false  -- Reset immediately
    ...
    if loadedSensorModule and loadedSensorModule.module.wakeup then
        ...
        if neuronsuite.session and neuronsuite.session.isConnected then
            -- run msp sensors
...
function sensors.getSid()
  ...
  sensors.sid = assert(neuronsuite.compiler.loadfile("tasks/sensors/sid.lua"))(config)
  return sensors.sid
end
