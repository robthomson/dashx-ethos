local folder = "flrtr"
local ESC = assert(neuronsuite.compiler.loadfile("app/modules/esc_tools/mfg/" .. folder .. "/init.lua"))()
...
if neuronsuite.session.escDetails and neuronsuite.session.escDetails.model then
    -- known models strings are
    -- FLYROTOR 280A
    -- FLYROTOR 150A

    -- note.  if you change the order of items in mspapi above - this will need to be updated
    if string.find(neuronsuite.session.escDetails.model, "FLYROTOR 150A") then
