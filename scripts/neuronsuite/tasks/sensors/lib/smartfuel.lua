...
local lastMode = neuronsuite.flightmode.current or "preflight" -- Last flight mode to detect changes.
...
local function smartFuelCalc()
    ...
    if currentMode ~= lastMode then
        neuronsuite.utils.log("Flight mode changed – resetting voltage & fuel state", "info")
   ...
   if stabilizeNotBefore and now < stabilizeNotBefore then
    ...
    local isDisarmed  = (neuronsuite and neuronsuite.session and neuronsuite.session.isArmed == false)
    local isPreflight = (neuronsuite and neuronsuite.flightmode and neuronsuite.flightmode.current == "preflight")
