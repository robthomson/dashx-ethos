...
function ui.progressDisplay(title, message, speed)
    if neuronsuite.app.dialogs.progressDisplay then return end
    ...
    if speed then
        neuronsuite.app.dialogs.progressSpeed = true
    else
        neuronsuite.app.dialogs.progressSpeed = false
    end

    neuronsuite.app.dialogs.progressDisplay   = true
    neuronsuite.app.dialogs.progressWatchDog  = os.clock()
    neuronsuite.app.dialogs.progress = form.openProgressDialog({
        title   = title,
        message = message,
        close   = function() end,
        wakeup  = function()
            local app = neuronsuite.app
            ...
            local apiV = tostring(neuronsuite.session.apiVersion)

            if not app.triggers.closeProgressLoader then
                app.dialogs.progressCounter = app.dialogs.progressCounter + (2 * mult)
                if app.dialogs.progressCounter > 50 and neuronsuite.session.apiVersion and not neuronsuite.utils.stringInArray(neuronsuite.config.supportedMspApiVersion, apiV) then
                    print("No API version yet")
                end
            elseif isProcessing then
                app.dialogs.progressCounter = app.dialogs.progressCounter + (3 * mult)
            elseif app.triggers.closeProgressLoader and neuronsuite.tasks.msp and neuronsuite.tasks.msp.mspQueue:isProcessed() then   -- this is the one we normally catch
            ...
            if app.dialogs.progressWatchDog
               and neuronsuite.tasks.msp
               and (os.clock() - app.dialogs.progressWatchDog) > tonumber(neuronsuite.tasks.msp.protocol.pageReqTimeout) 
            ...
            if not neuronsuite.tasks.msp  then
                app.dialogs.progressCounter = app.dialogs.progressCounter + (2 * mult)
                if app.dialogs.progressCounter >= 100 then
                    app.dialogs.progress:close()
                    app.dialogs.progressDisplay = false
    ...
    neuronsuite.app.dialogs.progressCounter = 0
    neuronsuite.app.dialogs.progress:value(0)
    neuronsuite.app.dialogs.progress:closeAllowed(false)
end
...
function ui.progressDisplaySave(message)
    ...
    neuronsuite.app.dialogs.saveDisplay  = true
    neuronsuite.app.dialogs.saveWatchDog = os.clock()
    ...
    neuronsuite.app.dialogs.save = form.openProgressDialog({
        title   = title,
        message = message,
        close   = function() end,
        wakeup  = function()
            ...
            elseif neuronsuite.tasks.msp.mspQueue:isProcessed() then
            ...
            local timeout = tonumber(neuronsuite.tasks.msp.protocol.saveTimeout + 5)
            if (app.dialogs.saveWatchDog and (os.clock() - app.dialogs.saveWatchDog) > timeout)
               or (app.dialogs.saveProgressCounter > 120 and neuronsuite.tasks.msp.mspQueue:isProcessed()) 
    ...
    neuronsuite.app.dialogs.save:value(0)
    neuronsuite.app.dialogs.save:closeAllowed(false)
end

-- Is any progress-related dialog showing?
function ui.progressDisplayIsActive()
    return neuronsuite.app.dialogs.progressDisplay
        or neuronsuite.app.dialogs.saveDisplay
        or neuronsuite.app.dialogs.progressDisplayEsc
        or neuronsuite.app.dialogs.nolinkDisplay
        or neuronsuite.app.dialogs.badversionDisplay
end
...
function ui.disableAllFields()
    for i = 1, #neuronsuite.app.formFields do
        local field = neuronsuite.app.formFields[i]
        if type(field) == "userdata" then field:enable(false) end
    end
end

function ui.enableAllFields()
    for _, field in ipairs(neuronsuite.app.formFields) do
        if type(field) == "userdata" then field:enable(true) end
    end
end

function ui.disableAllNavigationFields()
    for _, v in pairs(neuronsuite.app.formNavigationFields) do
        v:enable(false)
    end
end
...
function ui.enableNavigationField(x)
    local field = neuronsuite.app.formNavigationFields[x]
...
function ui.openMainMenu()
    ...
    neuronsuite.app.formFieldsOffline  = {}
    neuronsuite.app.formFieldsBGTask   = {}
    neuronsuite.app.formLines          = {}
    neuronsuite.app.lastLabel          = nil
    neuronsuite.app.isOfflinePage      = false

    if neuronsuite.tasks.msp then
        neuronsuite.tasks.msp.protocol.mspIntervalOveride = nil
    end

    neuronsuite.app.gfx_buttons["mainmenu"] = {}
    neuronsuite.app.lastMenu = nil

    -- Clear old icons.
    for k in pairs(neuronsuite.app.gfx_buttons) do
        if k ~= "mainmenu" then neuronsuite.app.gfx_buttons[k] = nil end
    end
    ...
    elseif neuronsuite.preferences.general.iconsize == 2 then
    ...
    neuronsuite.app.gfx_buttons["mainmenu"] = neuronsuite.app.gfx_buttons["mainmenu"] or {}
    neuronsuite.preferences.menulastselected["mainmenu"] =
        neuronsuite.preferences.menulastselected["mainmenu"] or 1

    local Menu = assert(neuronsuite.compiler.loadfile("app/modules/sections.lua"))()
    ...
    for pidx, pvalue in ipairs(Menu) do
        if not pvalue.developer then
            neuronsuite.app.formFieldsOffline[pidx] = pvalue.offline or false
            neuronsuite.app.formFieldsBGTask[pidx] = pvalue.bgtask or false
            ...
            if lc == 0 then
                y = form.height() +
                    ((neuronsuite.preferences.general.iconsize == 2) and neuronsuite.app.radio.buttonPadding
                                                                  or neuronsuite.app.radio.buttonPaddingSmall)
            end

            bx = (buttonW + padding) * lc

            if neuronsuite.preferences.general.iconsize ~= 0 then
                neuronsuite.app.gfx_buttons["mainmenu"][pidx] =
                    neuronsuite.app.gfx_buttons["mainmenu"][pidx] or lcd.loadMask(pvalue.image)
            else
                neuronsuite.app.gfx_buttons["mainmenu"][pidx] = nil
            end

            neuronsuite.app.formFields[pidx] = form.addButton(line, {
                x = bx, y = y, w = buttonW, h = buttonH
            }, {
                text    = pvalue.title,
                icon    = neuronsuite.app.gfx_buttons["mainmenu"][pidx],
                options = FONT_S,
                paint   = function() end,
                press   = function()
                    neuronsuite.preferences.menulastselected["mainmenu"] = pidx
                    local speed = false
                    if pvalue.loaderspeed then speed = true end
                    neuronsuite.app.ui.progressDisplay(nil,nil,speed)
                    if pvalue.module then
                        neuronsuite.app.isOfflinePage = true
                        neuronsuite.app.ui.openPage(pidx, pvalue.title, pvalue.module .. "/" .. pvalue.script)
                    else
                        neuronsuite.app.ui.openMainMenuSub(pvalue.id)
                    end
                end
            })
            ...
            if neuronsuite.preferences.menulastselected["mainmenu"] == pidx then
    ...
    collectgarbage()
    neuronsuite.utils.reportMemoryUsage("MainMenuSub")
end
...
function ui.openMainMenuSub(activesection)
    ...
    neuronsuite.app.lastLabel         = nil
    neuronsuite.app.isOfflinePage     = false
    neuronsuite.app.gfx_buttons[activesection] = {}
    neuronsuite.app.lastMenu = activesection

    -- Clear old icons.
    for k in pairs(neuronsuite.app.gfx_buttons) do
        if k ~= activesection then neuronsuite.app.gfx_buttons[k] = nil end
    end

    -- Hard exit on error.
    if not neuronsuite.utils.ethosVersionAtLeast(config.ethosVersion) then return end

    local MainMenu = neuronsuite.app.MainMenu

    -- Clear navigation vars.
    neuronsuite.app.lastIdx   = nil
    neuronsuite.app.lastTitle = nil
    neuronsuite.app.lastScript = nil
    neuronsuite.session.lastPage = nil
    neuronsuite.app.triggers.isReady             = false
    neuronsuite.app.uiState                      = neuronsuite.app.uiStatus.mainMenu
    neuronsuite.app.triggers.disableRssiTimeout  = false

    neuronsuite.preferences.general.iconsize = tonumber(neuronsuite.preferences.general.iconsize) or 1
    ...
    neuronsuite.app.gfx_buttons[activesection] = neuronsuite.app.gfx_buttons[activesection] or {}
    neuronsuite.preferences.menulastselected[activesection] =
        neuronsuite.preferences.menulastselected[activesection] or 1
    ...
    for idx, section in ipairs(MainMenu.sections) do
        if section.id == activesection then
            ...
            neuronsuite.app.formNavigationFields['menu'] = form.addButton(line, {
                x = x, y = neuronsuite.app.radio.linePaddingTop, w = 100, h = neuronsuite.app.radio.navbuttonHeight
            }, {
                text    = "MENU",
                icon    = nil,
                options = FONT_S,
            ...
            for pidx, page in ipairs(MainMenu.pages) do
                ...
                local hideEntry =
                    (page.ethosversion and not neuronsuite.utils.ethosVersionAtLeast(page.ethosversion))
                    or (page.mspversion and neuronsuite.utils.apiVersionCompare("<", page.mspversion))
                    or (page.developer and not neuronsuite.preferences.developer.devtools)

                    local offline = page.offline
                    neuronsuite.app.formFieldsOffline[pidx] = offline or false
                    ...
                    if not hideEntry then
                        ...
                        if neuronsuite.preferences.general.iconsize ~= 0 then
                            neuronsuite.app.gfx_buttons[activesection][pidx] =
                                neuronsuite.app.gfx_buttons[activesection][pidx]
                                or lcd.loadMask("app/modules/" .. page.folder .. "/" .. page.image)
                        else
                            neuronsuite.app.gfx_buttons[activesection][pidx] = nil
                        end
                        ...
                        neuronsuite.app.formFields[pidx] = form.addButton(line, {
                            ...
                            text    = page.title,
                            icon    = neuronsuite.app.gfx_buttons[activesection][pidx],
                            options = FONT_S,
                            paint   = function() end,
                            press   = function()
                                neuronsuite.preferences.menulastselected[activesection] = pidx
                                local speed = false
                                if page.loaderspeed or section.loaderspeed then speed = true end
                                neuronsuite.app.ui.progressDisplay(nil,nil,speed)
                                neuronsuite.app.isOfflinePage = offline
                                neuronsuite.app.ui.openPage(pidx, page.title, page.folder .. "/" .. page.script)
                            end
                        })

                        if neuronsuite.preferences.menulastselected[activesection] == pidx then
                            neuronsuite.app.formFields[pidx]:focus()
                        end
...
function ui.fieldBoolean(i)
    ...
    if f.inline and f.inline >= 1 and f.label then
        ...
        local p = neuronsuite.app.utils.getInlinePositions(f, page)
        posText, posField = p.posText, p.posField
        form.addStaticText(formLines[neuronsuite.app.formLineCnt], posText, f.t)
    else
        if f.t then
            if radioText == 2 and f.t2 then f.t = f.t2 end
        ...
        formLines[neuronsuite.app.formLineCnt] = form.addLine(f.t)
    ...
    formFields[i] = form.addBooleanField(
        formLines[neuronsuite.app.formLineCnt],
        posField,
        function()
            return decode()
        end,
        function(valueBool)
            ...
            f.value = neuronsuite.app.utils.saveFieldValue(fields[i], value)
        end
    )
...
function ui.fieldChoice(i)
    ...
    local tbldata = f.table and neuronsuite.app.utils.convertPageValueTable(f.table, f.tableIdxInc) or {}
    ...
    formFields[i] = form.addChoiceField(
        ...
        function()
            ...
            return neuronsuite.app.utils.getFieldValue(fields[i])
        end,
        function(value)
            if f.postEdit then f.postEdit(page, value) end
            if f.onChange then f.onChange(page, value) end
...
function ui.fieldSlider(i)
    ...
    local minValue = neuronsuite.app.utils.scaleValue(f.min, f)
    local maxValue = neuronsuite.app.utils.scaleValue(f.max, f)
    ...
    formFields[i] = form.addSliderField(
        ...
        function()
            ...
            return neuronsuite.app.utils.getFieldValue(page.fields[i])
        end,
        function(value)
            if f.postEdit then f.postEdit(page) end
            if f.onChange then f.onChange(page) end
            f.value = neuronsuite.app.utils.saveFieldValue(page.fields[i], value)
        end
    )
...
function ui.fieldStaticText(i)
    ...
    formFields[i] = form.addStaticText(
        formLines[neuronsuite.app.formLineCnt],
        posField,
        neuronsuite.app.utils.getFieldValue(fields[i])
    )
...
function ui.fieldLabel(f, i, l)
    ...
    if f.label then
        ...
        if f.label ~= neuronsuite.app.lastLabel then
            label.type = label.type or 0
            neuronsuite.app.formLineCnt = neuronsuite.app.formLineCnt + 1
            app.formLines[neuronsuite.app.formLineCnt] = form.addLine(labelName)
            form.addStaticText(app.formLines[neuronsuite.app.formLineCnt], nil, "")
            neuronsuite.app.lastLabel = f.label
        end
    end
end
...
function ui.fieldHeader(title)
    ...
    local formFields = app.formFields
    local lcdWidth  = neuronsuite.app.lcdWidth
...
local function getHelpData(section)
    if ui._helpCache[section] == nil then
        ...
        if neuronsuite.utils.file_exists(helpPath) then
            local ok, helpData = pcall(function()
                return assert(neuronsuite.compiler.loadfile(helpPath))()
            end)
...
function ui.openPage(idx, title, script, extra1, extra2, extra3, extra5, extra6)
    ...
    neuronsuite.app.Page = assert(neuronsuite.compiler.loadfile(modulePath))(idx)

    -- Load help (if present).
    local section  = script:match("([^/]+)")
    local helpData = getHelpData(section)
    neuronsuite.app.fieldHelpTxt = helpData and helpData.fields or nil

    -- Module-specific openPage?
    if neuronsuite.app.Page.openPage then
        neuronsuite.app.Page.openPage(idx, title, script, extra1, extra2, extra3, extra5, extra6)
        neuronsuite.utils.reportMemoryUsage(title)
        return
    end
    ...
    if neuronsuite.app.Page.headerLine then
        ...
        form.addStaticText(headerLine, {
            x = 0, y = neuronsuite.app.radio.linePaddingTop,
            w = neuronsuite.app.lcdWidth, h = neuronsuite.app.radio.navbuttonHeight
    ...
    if neuronsuite.app.Page.fields then
        for i, field in ipairs(neuronsuite.app.Page.fields) do
            local label   = neuronsuite.app.Page.labels
            if neuronsuite.session.apiVersion == nil then return end

            local valid =
                (field.apiversion    == nil or neuronsuite.utils.apiVersionCompare(">=", field.apiversion))    and
                (field.apiversionlt  == nil or neuronsuite.utils.apiVersionCompare("<",  field.apiversionlt))  and
                (field.apiversiongt  == nil or neuronsuite.utils.apiVersionCompare(">",  field.apiversiongt))  and
                (field.apiversionlte == nil or neuronsuite.utils.apiVersionCompare("<=", field.apiversionlte)) and
                (field.apiversiongte == nil or neuronsuite.utils.apiVersionCompare(">=", field.apiversiongte)) and
            ...
            if field.hidden ~= true and valid then
                ...
                if     field.type == 0 then neuronsuite.app.ui.fieldStaticText(i)
                elseif field.table or field.type == 1 then neuronsuite.app.ui.fieldChoice(i)
                elseif field.type == 2 then neuronsuite.app.ui.fieldNumber(i)
                elseif field.type == 3 then neuronsuite.app.ui.fieldText(i)
                elseif field.type == 4 then neuronsuite.app.ui.fieldBoolean(i)
                elseif field.type == 5 then neuronsuite.app.ui.fieldBooleanInverted(i)  
                elseif field.type == 6 then neuronsuite.app.ui.fieldSlider(i)  
                elseif field.type == 7 then neuronsuite.app.ui.fieldSource(i)   
                elseif field.type == 8 then neuronsuite.app.ui.fieldSwitch(i) 
                elseif field.type == 9 then neuronsuite.app.ui.fieldSensor(i)     
                elseif field.type == 10 then neuronsuite.app.ui.fieldColor(i) 
                else                         neuronsuite.app.ui.fieldNumber(i)
                end
            else
...
function ui.navigationButtons(x, y, w, h)
    ...
    local navButtons
    if neuronsuite.app.Page.navButtons == nil then
        navButtons = { menu = true, save = true, reload = true, help = true }
    else
        navButtons = neuronsuite.app.Page.navButtons
    end
    ...
    if navButtons.menu == true then
        neuronsuite.app.formNavigationFields['menu'] = form.addButton(line, {
            ...
            press   = function()
                if neuronsuite.app.Page and neuronsuite.app.Page.onNavMenu then
                    neuronsuite.app.Page.onNavMenu(neuronsuite.app.Page)
                elseif neuronsuite.app.lastMenu ~= nil then
    ...
    if navButtons.save == true then
        neuronsuite.app.formNavigationFields['save'] = form.addButton(line, {
            ...
            paint   = function() end,
            press   = function()
                if neuronsuite.app.Page and neuronsuite.app.Page.onSaveMenu then
                    neuronsuite.app.Page.onSaveMenu(neuronsuite.app.Page)
                else
                    neuronsuite.app.triggers.triggerSave = true
                end
            end
        })
    end
    ...
    if navButtons.reload == true then
        neuronsuite.app.formNavigationFields['reload'] = form.addButton(line, {
            ...
            paint   = function() end,
            press   = function()
                if neuronsuite.app.Page and neuronsuite.app.Page.onReloadMenu then
                    neuronsuite.app.Page.onReloadMenu(neuronsuite.app.Page)
                else
                    neuronsuite.app.triggers.triggerReload = true
                end
                return true
            end
        })
    ...
    if navButtons.tool == true then
        neuronsuite.app.formNavigationFields['tool'] = form.addButton(line, {
            ...
            paint   = function() end,
            press   = function()
                neuronsuite.app.Page.onToolMenu()
            end
        })
    end
    ...
    if navButtons.help == true then
        local section = neuronsuite.app.lastScript:match("([^/]+)")
        local script  = neuronsuite.app.lastScript:match("/([^/]+)%.lua$")

        local help = getHelpData(section)
        if help then
            neuronsuite.app.formNavigationFields['help'] = form.addButton(line, {
                ...
                paint   = function() end,
                press   = function()
                    if neuronsuite.app.Page and neuronsuite.app.Page.onHelpMenu then
                        neuronsuite.app.Page.onHelpMenu(neuronsuite.app.Page)
                    else
                        if help.help[script] then
                            neuronsuite.app.ui.openPageHelp(help.help[script], section)
                        else
                            neuronsuite.app.ui.openPageHelp(help.help['default'], section)
                        end
                    end
                end
            })
            ...
            neuronsuite.app.formNavigationFields['help']:enable(false)
        end
    end
end
...
function ui.openPageHelp(txtData, section)
    ...
    form.openDialog({
        width   = neuronsuite.app.lcdWidth,
        title   = "Help - " .. neuronsuite.app.lastTitle,
        message = message,
...
function ui.injectApiAttributes(formField, f, v)
    ...
    if v.table and not f.table then
        ...
        local tbldata = neuronsuite.app.utils.convertPageValueTable(v.table, idxInc)
        if f.type == 1 then
